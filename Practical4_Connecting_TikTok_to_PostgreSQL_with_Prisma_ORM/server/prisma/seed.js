...
async function main() {
  ...
  for (let u = 0; u < users.length; u++) {
    for (let j = 1; j <= 5; j++) {
      const video = await prisma.video.create({
        data: {
          userId: users[u].id,
          caption: `Video ${j} from user ${users[u].username}`,
          videoUrl: `https://example.com/videos/user${users[u].id}_video${j}.mp4`,
          thumbnailUrl: `https://example.com/thumbnails/user${users[u].id}_video${j}.jpg`,
          audioName: `Original Sound - User ${users[u].id}`,
  ...
  for (let i = 0; i < 40; i++) {
    ...
    try {
      ...
      console.log(`Created follow: User ${users[followerIndex].id} follows User ${users[followingIndex].id}`);
    } catch (error) {
      console.log(`Skipping duplicate follow: User ${users[followerIndex].id} -> User ${users[followingIndex].id}`);
    }
  }
